  ...
  const getQuarterlyData = (data) => {
    ...
    (data || []).forEach(({ month, value }) => {
      if (["January", "February", "March"].includes(month))
        quarterlySums.Q1 += value;
      if (["April", "May", "June"].includes(month)) quarterlySums.Q2 += value;
      if (["July", "August", "September"].includes(month))
        quarterlySums.Q3 += value;
      if (["October", "November", "December"].includes(month))
        quarterlySums.Q4 += value;
    });
