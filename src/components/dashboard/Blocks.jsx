...
const Blocks = ({ setSelectedSection, selectedTab }) => {
    ...
    const [dateMode, setDateMode] = useState('single'); // 'single', 'range', 'month', 'quarter', 'custom'
    ...
    useEffect(() => {
        const fetchDashboardData = async () => {
            try {
                ...
                let apiUrl = `${BASE_URL}/api/customer/dashboard-blocks`;
                let params = {};
                ...
                const { activeCentreCount = 0, inactiveCentreCount = 0 } = centerStatusResponse.data;
        ...
        fetchDashboardData();
    }, [selectedTab, dateMode, selectedDate, startDate, endDate, selectedMonth, selectedQuarter, customStartDate, customEndDate]);
        ...
        <div className="w-full mt-10 pr-15 font-[Plus] h-auto">
            ...
            <div className="grid grid-cols-4 justify-start items-center gap-6">
                ...
                {loading ? (
                    ...
                    blocksData
                        .filter((block) => {
                            return !(selectedTab === "inactive" && block.title === "Center Status");
                        })
                        .map((block) => {
                            ...
                            switch (block.title) {
                                ...
                                case "This Month's Customers":
                                case "Quarter Q1 Customers":
                                case "Quarter Q2 Customers":
                                case "Quarter Q3 Customers":
                                case "Quarter Q4 Customers":
                                case "Filtered Range Customers":
                                    targetSection = "customers";
                                    displayTitle = `${dateText} Customers`;
                                    break;
                                case "Today's Online Payments":
                                case "This Month's Online Payments":
                                case "Quarter Q1 Online Payments":
                                case "Quarter Q2 Online Payments":
                                case "Quarter Q3 Online Payments":
                                case "Quarter Q4 Online Payments":
                                case "Filtered Range Online Payments":
                                    targetSection = "online-collection";
                                    displayTitle = `${dateText} Online Payments`;
                                    break;
                                case "Today's Cash Payments":
                                case "This Month's Cash Payments":
                                case "Quarter Q1 Cash Payments":
                                case "Quarter Q2 Cash Payments":
                                case "Quarter Q3 Cash Payments":
                                case "Quarter Q4 Cash Payments":
                                case "Filtered Range Cash Payments":
                                    targetSection = "/";
                                    displayTitle = `${dateText} Cash Payments`;
                                    break;
                                case "Today's Total Payments":
                                case "This Month's Total Payments":
                                case "Quarter Q1 Total Payments":
                                case "Quarter Q2 Total Payments":
                                case "Quarter Q3 Total Payments":
                                case "Quarter Q4 Total Payments":
                                case "Filtered Range Total Payments":
                                    targetSection = "total-collection";
                                    displayTitle = `${dateText} Total Payments`;
                                    break;
                                case "Today's Commission":
                                case "This Month's Commission":
                                case "Quarter Q1 Commission":
                                case "Quarter Q2 Commission":
                                case "Quarter Q3 Commission":
                                case "Quarter Q4 Commission":
                                case "Filtered Range Commission":
                                ...
                                case "Center Status":
                                    ...
                                    // Store the current date selection for center status
                                    if (dateMode === 'single') {
                                        localStorage.setItem("centerStatusDate", formatDateForAPI(selectedDate));
                                    } else if (dateMode === 'range') {
                                        localStorage.setItem("centerStatusStartDate", formatDateForAPI(startDate));
                                        localStorage.setItem("centerStatusEndDate", formatDateForAPI(endDate));
                                    } else if (dateMode === 'month') {
                                        localStorage.setItem("centerStatusMonth", selectedMonth);
                                    } else if (dateMode === 'quarter') {
                                        localStorage.setItem("centerStatusQuarter", selectedQuarter);
                                    } else if (dateMode === 'custom') {
                                        localStorage.setItem("centerStatusStartDate", formatDateForAPI(customStartDate));
                                        localStorage.setItem("centerStatusEndDate", formatDateForAPI(customEndDate));
                                    }
                                    break;
                                case "Area Manager Collection":
                            ...
                            return (
                                ...
                                >
                                    <h3 className="text-2xl mb-6 text-[#91929E]">{block.value}</h3>
