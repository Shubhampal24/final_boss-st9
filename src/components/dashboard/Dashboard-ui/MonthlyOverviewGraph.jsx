...
const MonthlyOverviewGraph = () => {
                ...
                <div className="bg-white p-4 border border-gray-300 shadow-lg rounded-lg">
                    ...
                    {payload.map((entry, index) => (
                        <p key={index} style={{ color: entry.color }} className="font-semibold">
                            {entry.name}: {entry.dataKey === 'customers'
    ...
    useEffect(() => {
        const fetchMonthlyData = async () => {
            ...
            try {
                setLoading(true);
                const response = await axios.get(`${BASE_URL}/api/customer/revenue?year=${selectedYear}`, {
                ...
                if (response.data && response.data.monthlyRevenue) {
                    response.data.monthlyRevenue.forEach(item => {
                        const monthNum = parseInt(item._id.split('-')[1], 10);
    ...
    const totalAnnualRevenue = monthlyData.reduce((sum, item) => sum + item.revenue, 0);
    const totalCustomers = monthlyData.reduce((sum, item) => sum + (item.customers || 0), 0);

    const highestRevenueMonth = monthlyData.length > 0
        ? monthlyData.reduce((max, item) => item.revenue > max.revenue ? item : max)
        : { month: 'N/A', revenue: 0 };

    let lowestRevenueMonth = monthlyData.length > 0
        ? monthlyData.filter(item => item.revenue > 0).reduce(
            (min, item) => item.revenue < min.revenue ? item : min,
            { month: 'N/A', revenue: Number.MAX_SAFE_INTEGER }
        )
        : { month: 'N/A', revenue: 0 };
        ...
        <div className='w-full py-10 px-4 sm:px-6 lg:px-32'>
            <div className='max-w-7xl mx-auto'>
                <div className='mb-8 text-center'>
                    <h2 className='text-2xl font-bold text-gray-100 mb-2'>Revenue Overview</h2>
                    <p className='text-gray-100 max-w-2xl mx-auto py-5'>
                        Monthly revenue breakdown for the year, showcasing financial performance across different months.
                    </p>
                </div>
                ...
                <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 mb-10 text-center'>
                    <div className='bg-[#1F1F24] p-6 rounded-lg shadow-md'>
                        <h3 className='text-lg font-semibold text-gray-200 mb-2'>Total Annual Revenue</h3>
                        <p className='text-2xl font-bold text-green-600'>₹ {shortenNumber(totalAnnualRevenue)}</p>
                    </div>
                    <div className='bg-[#1F1F24] p-6 rounded-lg shadow-md'>
                        <h3 className='text-lg font-semibold text-gray-200 mb-2'>Total Customers</h3>
                        <p className='text-2xl font-bold text-orange-600'>{shortenNumber(totalCustomers)}</p>
                    </div>
                    <div className='bg-[#1F1F24] p-6 rounded-lg shadow-md'>
                        <h3 className='text-lg font-semibold text-gray-200 mb-2'>Highest Revenue Month</h3>
                        <p className='text-xl font-bold text-blue-600'>
                            {highestRevenueMonth.month}
                        </p>
                    </div>
                    <div className='bg-[#1F1F24] p-6 rounded-lg shadow-md'>
                        <h3 className='text-lg font-semibold text-gray-200 mb-2'>Lowest Revenue Month</h3>
                        <p className='text-xl font-bold text-red-500'>{lowestRevenueMonth.month}</p>
                    </div>
                </div>
                ...
                <div className='bg-[#1F1F24] p-4 sm:p-6 md:p-8 rounded-lg shadow-lg'>
                    <ResponsiveContainer width="100%" height={400}>
                        ...
                        >
                            ...
                            <YAxis
                                ...
                                label={{
                                    value: activeDataView === 'customers' ? 'Customers' : 'Revenue (₹)',
                                    angle: -90,
                                    position: 'insideLeft',
                    ...
                    <div className="flex flex-wrap justify-center mt-6 gap-4">
                        <button
                            className={`px-4 py-2 text-sm sm:text-base ${activeDataView === 'totalRevenue' ? 'bg-purple-600' : 'bg-gray-800'} text-white rounded border border-gray-600 hover:bg-gray-700`}
                        ...
                        <button
                            className={`px-4 py-2 text-sm sm:text-base ${activeDataView === 'customers' ? 'bg-purple-600' : 'bg-gray-800'} text-white rounded border border-gray-600 hover:bg-gray-700`}
                            onClick={() => handleDataViewChange('customers')}>
                            Customers
                        </button>
                        <button
                            className={`px-4 py-2 text-sm sm:text-base ${activeDataView === 'totalCash' ? 'bg-purple-600' : 'bg-gray-800'} text-white rounded border border-gray-600 hover:bg-gray-700`}
                            onClick={() => handleDataViewChange('totalCash')}>
                            Total Cash
                        </button>
                        <button
                            className={`px-4 py-2 text-sm sm:text-base ${activeDataView === 'totalOnline' ? 'bg-purple-600' : 'bg-gray-800'} text-white rounded border border-gray-600 hover:bg-gray-700`}
