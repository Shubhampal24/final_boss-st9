...
const SummaryItem = ({ title, value, payCriteria }) => {
    ...
    return (
        <>
            <div className={`border border-gray-700 ${getBgColor()} p-2`}>{title}</div>
                    ...
                    <>
                        ...
                        <div className="space-y-4 max-h-[65vh] overflow-y-auto pr-3 mb-6">
                            <div className="flex justify-between bg-gray-900 p-3 rounded">
                                <span className="font-semibold text-gray-400">Expense Date:</span>
                            ...
                            <div className="flex items-center gap-2 text-white">
                                <input type="checkbox" checked={verified} readOnly className="w-4 h-4" />
                                <span>Verified</span>
                            </div>
                        </div>
...
const ReportPopup = ({ selectedReport, closeModal }) => {
    ...
    useEffect(() => {
        if (selectedReport?.report?.centerDetails?._id) {
            setCenterIds([selectedReport.report.centerDetails._id]);
        }
    }, [selectedReport]);
    ...
    useEffect(() => {
        if (selectedDate) {
            setFilteredCustomers(customers.filter(customer =>
                customer?.inTime?.slice(0, 10) === selectedDate
            ));
        } else {
    ...
    const handleUpdateCustomer = async (e) => {
        ...
        try {
            ...
            await axios.put(
                `${API_BASE_URL}/api/customer/update/${editingCustomer._id}`,
                editFormData,
                {
                    headers: {
    ...
    const handleUpdateExpense = async (e) => {
        ...
        try {
            ...
            // Make API request with authorization header
            await axios.put(
                `${API_BASE_URL}/api/expense/${editingExpense._id}`,
                expenseFormData,
                {
                    headers: {
                        'Authorization': `Bearer ${token}`
    ...
    const overallExpenses = getSummaryValue(selectedReport.report?.overallExpenses);
    const balance = getSummaryValue(selectedReport.report?.centerDetails.balance + selectedReport.report?.centerDetails.previousBalance - selectedReport.report?.overallExpenses);
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
            <div className="bg-[#121212] text-white p-6 rounded-lg w-full max-w-[90%] max-h-[90vh] overflow-y-auto">
                ...
                <div className="w-full flex mb-10 justify-between items-center">
                    <h2 className="text-2xl font-bold mb-4 text-center">
                        {selectedReport.report?.centerDetails?.centreId
                            ? `${selectedReport.report.centerDetails.centreId} (ID: ${selectedReport.report.centerDetails._id})`
                            : "No Center Name"}
                    </h2>
                ...
                <div className="flex flex-col md:flex-row gap-4 mb-6">
                    <PendingEntriesCounter customers={filteredCustomers} selectedDate={selectedDate} />
                    <RaisedIssuesCounter customers={filteredCustomers} selectedDate={selectedDate} />
                </div>
                        ...
                        <tbody>
                            {Array.isArray(filteredCustomers) && filteredCustomers.length > 0 ? (
                                filteredCustomers.map((customer, index) => (
                                    <tr key={customer._id || index} className="border text-center border-gray-700">
                                        <td className="border text-center px-4 py-2">{index + 1}</td>
                                        <td className="border text-center px-4 py-2">{customer?.name || "N/A"}</td>
                                        <td className="border text-center px-4 py-2">{customer?.number || "N/A"}</td>
                                        <td className="border text-center px-4 py-2">{customer?.duration || "N/A"}</td>
                                        <td className="border text-center px-4 py-2">
                                            {customer?.inTime ? new Date(customer.inTime).toLocaleString('en-IN', {
                                                day: '2-digit', month: '2-digit', year: 'numeric',
                                                hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: true,
                                            }) : "N/A"}
                                        </td>
                                        <td
                                            className={`border text-center px-4 py-2 ${!customer?.outTime ? 'bg-red-100 text-red-800 font-semibold' : ''
                                                }`}
                                        >
                                            {customer?.outTime ? new Date(customer.outTime).toLocaleString('en-IN', {
                                                day: '2-digit',
                                                month: '2-digit',
                                                year: 'numeric',
                                                hour: '2-digit',
                                        ...
                                        <td className="border text-center px-4 py-2">
                                            {Array.isArray(customer?.staffAttending) && customer.staffAttending.length > 0
                                                ? customer.staffAttending.map(staff => staff.name).join(", ")
                                                : "N/A"}
                                        </td>
                                        <td className="border text-center px-4 py-2">{(customer?.paymentCash1 || 0) + (customer?.paymentCash2 || 0)}</td>
                                        <td className="border text-center px-4 py-2">{(customer?.paymentOnline1 || 0) + (customer?.paymentOnline2 || 0)}</td>
                                        <td className="border text-center px-4 py-2">{customer?.cashCommission || 0}</td>
                                        <td className="border text-center px-4 py-2">{customer?.onlineCommission || 0}</td>
                                        <td className="border text-center px-4 py-2">
                                            {customer?.status &&
                                                customer.status.toLowerCase() !== "all ok" &&
                                                customer.status.toLowerCase() !== "null" ? (
                                                <button
                                                    onClick={() => handleIssueToggle(customer.remark || "No remark")}
                                                ...
                                                >
                                                    ...
                                                    <svg
                                                        ...
                                                        strokeWidth={2}
                                                    >
                                                        <path strokeLinecap="round" strokeLinejoin="round" d="M10.29 3.86L1.82 18a1.007 1.007 0 00.86 1.5h18.64a1.007 1.007 0 00.86-1.5L13.71 3.86a1 1 0 00-1.42 0z" />
                                        ...
                                        <td className="border text-center px-4 py-2">
                                            {customer?._id && (
                                                <div className="flex gap-2 justify-center">
                                                    ...
                                                    {!customer.outTime ? (
                                                    ...
                                                    ) : Date.now() - new Date(customer.outTime).getTime() <= 3 * 24 * 60 * 60 * 1000 ? (
                                                        <button
                                                            className="bg-blue-600 text-white px-2 py-1 rounded hover:bg-blue-700"
                                                            onClick={() => handleEditClick(customer)}
                                                        ...
                                                        >
                                                            <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 text-gray-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 17a1 1 0 001 1h6a1 1 0 001-1V7a1 1 0 00-1-1h-6a1 1 0 00-1 1v10zm-4-8v-2a4 4 0 118 0v2" />
                                                            </svg>
                                                            Edit
                                                        </button>
                                                    )}
                ...
                {showIssuePopup && (
                    <div className="fixed inset-0 z-[70] flex items-center justify-center pointer-events-none">
                        <div className="pointer-events-auto bg-[#1E1E1E]/90 backdrop-blur-sm p-6 rounded-lg w-[400px] shadow-lg border border-gray-600">
                            <h3 className="text-xl font-bold mb-4 text-white">Issue Remark</h3>
                            <p className="mb-6 whitespace-pre-wrap text-gray-200">{issueRemark}</p>
                ...
                <div className="w-full flex gap-4 mt-10">
                    <div className="w-full lg:w-1/2">
                        ...
                        <div className="mt-4 bg-[#6F5FE7] p-2 font-bold">Expenditure Details</div>
                                    ...
                                    {Array.isArray(expenses) && expenses.length > 0 ? (
                                        expenses.map((expense, index) => (
                                            <tr key={expense._id || index}>
                                                <td className="border text-center px-4 py-2">{expense?.reason || "N/A"}</td>
                                                <td className="border text-center px-4 py-2">{expense?.amount || 0}</td>
                                                <td className="border text-center px-4 py-2">
                                                ...
                                                <td className="border text-center px-4 py-2">
                                                    {expense?._id && (
                                                        !expense.expenseDate ||
                                                            (Date.now() - new Date(expense.expenseDate).getTime() <= 3 * 24 * 60 * 60 * 1000
                                                            ) ? (
                                                            <button
                                                                className="bg-blue-600 text-white px-2 py-1 rounded hover:bg-blue-700"
                    ...
                    <div className="mt-4 w-1/2 grid grid-cols-2 gap-2">
                        <SummaryItem title="Total Business (onl+csh)- com" value={totalBusiness} />
                        <SummaryItem title="Total Online (Onlinecust+Onlinecom)" value={totalOnline} />
                        ...
                        {selectedReport.report?.centerDetails?.payCriteria !== "minus" && (
                            <>
                                <SummaryItem
                                    title={`Today's Final Cash ((cashCust+CashComm)-exp) (${selectedReport.report?.centerDetails?.payCriteria || 'N/A'})`}
                                    value={finalTotal}
                                    payCriteria={selectedReport.report?.centerDetails?.payCriteria}
                                />
                            </>
                ...
                <div className="flex justify-end mt-6">
                    <button className="bg-red-600 cursor-pointer px-4 py-2 rounded-lg" onClick={closeModal}>
                        Close
                    </button>
                </div>
                {showDeleteConfirm && (
                                ...
                                <>
                                    ...
                                    <div className="space-y-4 max-h-[65vh] overflow-y-auto pr-3">
                                        <div className="flex justify-between bg-gray-900 p-3 rounded">
                                            ...
                                            <span className="text-white">{editFormData.name || "-"}</span>
                                        </div>
                                        <div className="flex justify-between bg-gray-900 p-3 rounded">
                                            <span className="font-semibold text-gray-400">Contact Number:</span>
                                            <span className="text-white">{editFormData.number || "-"}</span>
                                        </div>
                                        <div className="flex justify-between bg-gray-900 p-3 rounded">
                                            <span className="font-semibold text-gray-400">Duration:</span>
                                            <span className="text-white">{editFormData.duration || "-"}</span>
                                        </div>
                                        <div className="flex justify-between bg-gray-900 p-3 rounded">
                                            <span className="font-semibold text-gray-400">Cash Payment 1:</span>
                                            <span className="text-white">{editFormData.paymentCash1 ?? "-"}</span>
                                        </div>
                                        <div className="flex justify-between bg-gray-900 p-3 rounded">
                                            <span className="font-semibold text-gray-400">Cash Payment 2:</span>
                                            <span className="text-white">{editFormData.paymentCash2 ?? "-"}</span>
                                        </div>
                                        <div className="flex justify-between bg-gray-900 p-3 rounded">
                                            <span className="font-semibold text-gray-400">Online Payment 1:</span>
                                            <span className="text-white">{editFormData.paymentOnline1 ?? "-"}</span>
                                        </div>
                                        <div className="flex justify-between bg-gray-900 p-3 rounded">
                                            <span className="font-semibold text-gray-400">Online Payment 2:</span>
                                            <span className="text-white">{editFormData.paymentOnline2 ?? "-"}</span>
                                        </div>
                                        <div className="flex justify-between bg-gray-900 p-3 rounded">
                                            <span className="font-semibold text-gray-400">Cash Commission:</span>
                                            <span className="text-white">{editFormData.cashCommission ?? "-"}</span>
                                        </div>
                                        <div className="flex justify-between bg-gray-900 p-3 rounded">
                                            <span className="font-semibold text-gray-400">Online Commission:</span>
                                            <span className="text-white">{editFormData.onlineCommission ?? "-"}</span>
                                        </div>
                                    </div>
                                    <div className="flex justify-end gap-4 mt-6">
                                        <button
                    ...
                    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black bg-opacity-50">
                        <div className="bg-[#1E1E1E] p-6 rounded-lg w-[400px] max-h-[90vh] overflow-y-auto">
                            ...
                            {modalStep === "edit" ? (
                                ...
                                // Summary View
                                <>
                                    <h3 className="text-2xl font-bold mb-6 border-b border-gray-700 pb-2">Summary of Changes</h3>
                                    {/* Display data plainly or styled as needed */}
                                    <div className="space-y-4 max-h-[65vh] overflow-y-auto pr-3">
                                        <div className="flex justify-between bg-gray-900 p-3 rounded">
                                            <span className="font-semibold text-gray-400">Description:</span>
                                            <span className="text-white">{expenseFormData.reason || "-"}</span>
                                        </div>
                                        <div className="flex justify-between bg-gray-900 p-3 rounded">
                                            <span className="font-semibold text-gray-400">Amount:</span>
                                            <span className="text-white">{expenseFormData.amount ?? "-"}</span>
                                        </div>
                                        <div className="flex justify-between bg-gray-900 p-3 rounded">
                                            <span className="font-semibold text-gray-400">Date:</span>
                                            <span className="text-white">{expenseFormData.expenseDate || "-"}</span>
                                        </div>
                                    </div>
