...
const Drawer = ({ isOpen, onClose }) => {
  ...
  return (
    <div
      className={`fixed top-0 right-0 min-h-screen overflow-y-auto h-full px-5 w-1/4 rounded-l-4xl bg-[#1F1F24] shadow-lg transform ${
    ...
    >
      <div className="p-6 text-white">
        <button
          onClick={onClose}
          className="text-white cursor-pointer text-xl absolute top-4 right-4"
        >
          ✖
        </button>
        ...
        <form className="flex flex-col gap-4">
          ...
          <div className="w-full relative">
            <Listbox
              ...
              onChange={(value) => handleMultiSelectChange(value, "regionIds")}
              multiple
            >
              <Listbox.Button className="h-auto py-4 px-4 w-full text-left text-white border rounded-xl">
                {formData.regionIds.length > 0
                  ? formData.regionIds
                      .map(
                        (id) => regions.find((region) => region.id === id)?.name
                      )
                      .join(", ")
                  : "Select Regions"}
              </Listbox.Button>
          ...
          <div className="w-full relative">
            <Listbox
              ...
              onChange={(value) => handleMultiSelectChange(value, "branchIds")}
              multiple
            >
              <Listbox.Button className="h-auto py-4 px-4 w-full text-left text-white border rounded-xl">
          ...
          <div className="w-full relative">
            <Listbox
              ...
              onChange={(value) => handleMultiSelectChange(value, "centreIds")}
              multiple
            >
              <Listbox.Button className="h-auto py-4 px-4 w-full text-left text-white border rounded-xl">
                {formData.centreIds.length > 0
                  ? formData.centreIds
                      .map(
                        (id) => centers.find((center) => center.id === id)?.name
                      )
                      .join(", ")
                  : "Select Centers"}
              </Listbox.Button>
          ...
          <button
            ...
            type="submit"
            className="bg-[#6F5FE7] p-2 py-4 rounded-2xl text-white font-semibold"
          >
            Add Staff
          </button>
        </form>
