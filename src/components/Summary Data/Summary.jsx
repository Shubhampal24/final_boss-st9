...
const DailySummary = () => {
    ...
    const formatExpenseReasons = (data) => {
        if (data.expenseReasonsDateWise && typeof data.expenseReasonsDateWise === 'object' && data.expenseReasonsDateWise !== null) {
  ...
  autoTable(doc, {
    ...
    const downloadCentreReport = async (centreId) => {
        ...
        try {
            ...
            const tableColumn = [
                "Sr No.", "Date", "Net Revenue\n(Business)", "Customer\nCount",
                "Cash\nBus.", "Online\n(Bus.)", "Online\nCom.", "Total\nOnline",
                "Cash\nCom.", "Total\nPayCom", "Expenses", "Exp. Reasons",
            ];
            ...
            const tableRows = dataArr.map((item, index) => {
                ...
                return [
                    index + 1,
                    item.istDateString ? new Date(item.istDateString).toLocaleDateString("en-IN") : "N/A",
            ...
            const totals = dataArr.reduce(
                ...
                {
                    totalCash: 0, totalOnline: 0, totalCashCommission: 0,
                    totalOnlineCommission: 0, grandtotalonline: 0, totalPayCom: 0,
                    totalExpense: 0, totalCustomerCount: 0, totalRevenue: 0,
                }
            );
           ...
           const payCriteria = dataArr[0]?.centreId?.payCriteria;
            ...
            const fileName = `Centre-Report-${dataArr[0].centreId.centreId}-${filterType === "date" ? dateFilter : monthFilter}.pdf`;
            doc.save(fileName);
    ...
    useEffect(() => {
        if ((filterType === "date" && dateFilter) || (filterType === "month" && monthFilter)) {
            fetchDailySummaries();
        }
    ...
    const calculateTotals = () => {
        return summaries.reduce(
            (acc, summary) => {
                ...
                acc.grandtotalonline += (summary.totalOnline || 0) + (summary.totalOnlineCommission || 0);
                acc.totalPayCom += (summary.totalOnlineCommission || 0) + (summary.totalCashCommission || 0);
                acc.totalExpense += summary.totalExpense || 0;
                acc.totalCustomerCount += summary.customerCount || 0;
                acc.totalRevenue += (summary.totalCash || 0) + (summary.totalOnline || 0);
                return acc;
            },
            {
                totalCash: 0, totalOnline: 0, totalCashCommission: 0,
    ...
    const downloadPDF = () => {
        ...
        try {
            ...
            doc.setFontSize(12);
            const filterText = filterType === "date" ? `Date: ${dateFilter}` : `Month: ${monthFilter}`;
            doc.text(filterText, 14, 30);

            const tableColumn = [
                "Sr No.", "Centre ID", "Centre Name", "Net Revenue\n(Business)",
                "Customer\nCount", "Cash\nBus.", "Online\n(Bus.)", "Online\nCom.",
                "Total\nOnline", "Cash\nCom.", "Total\nPayCom", "Expenses", "Exp. Reasons",
            ];

            const tableRows = filteredSummaries.map((summary, index) => {
                const netRevenue = (summary.totalCash || 0) + (summary.totalOnline || 0);
                return [
                    index + 1,
                    ...
                    "Rs. " + formatCurrencyForPDF((summary.totalOnline || 0) + (summary.totalOnlineCommission || 0)),
                    "Rs. " + formatCurrencyForPDF(summary.totalCashCommission),
                    "Rs. " + formatCurrencyForPDF((summary.totalOnlineCommission || 0) + (summary.totalCashCommission || 0)),
            ...
            const fileName = filterType === "date"
        ...
        <div className="w-full py-5 min-h-screen h-auto bg-[#0D0D11] px-13 text-white">
            ...
            <div className="w-full mt-6 h-auto flex flex-col md:flex-row md:justify-between md:items-center gap-4">
                ...
                <div className="flex flex-col md:flex-row items-center gap-4 w-full md:w-auto">
                    ...
                    {filterType === "date" && (
                        <>
                            <label htmlFor="dateFilter" className="text-sm font-semibold">Select Date:</label>
                            <input
                                type="date"
                                id="dateFilter"
                                value={dateFilter}
                    ...
                    {filterType === "month" && (
                        <>
                            <label htmlFor="monthFilter" className="text-sm font-semibold">Select Month:</label>
                            <input
                                type="month"
                                id="monthFilter"
                                value={monthFilter}
            ...
            {filteredSummaries.length === 0 ? (
                <div className="mt-10 p-6 text-center bg-[#1E1E24] rounded-lg shadow-md text-gray-400">
                    <p className="text-xl mb-2">
                        No summary data available for {filterType === "date" ? dateFilter : monthFilter}
                    </p>
                    <p>Try selecting a different {filterType} or check if data has been submitted.</p>
                </div>
            ) : (
                        ...
                        <table className="w-full text-left border border-gray-600">
                            ...
                            <tbody>
                                {filteredSummaries.map((summary, index) => {
                                    const netRevenue = (summary.totalCash || 0) + (summary.totalOnline || 0);
                                    return (
                                        <tr key={summary._id || summary.id} className="border text-center border-gray-600">
                                            <td className="py-3 px-4 border">{index + 1}</td>
                                            <td className="py-3 px-4 border">{summary.centreId.centreId?.toString().slice(0, 3)}</td>
                                            <td className="py-3 px-4 border">{summary.centreId.name}</td>
                                            <td className={`py-3 px-4 border ${netRevenue >= 0 ? "text-green-400" : "text-red-400"}`}>
                                                {formatCurrency(netRevenue)}
                                            </td>
                                            <td className="py-3 px-4 border">{formatCurrency(summary.totalCash)}</td>
                                            <td className="py-3 px-4 border">{formatCurrency(summary.totalOnline)}</td>
                                            <td className="py-2 px-3 border">{formatCurrency(summary.totalOnlineCommission)}</td>
                                            <td className="py-2 px-3 border">
                                                {formatCurrency((summary.totalOnline || 0) + (summary.totalOnlineCommission || 0))}
                                            </td>
                                            <td className="py-3 px-4 border">{formatCurrency(summary.totalCashCommission)}</td>
                                            <td className="py-2 px-3 border">
                                                {formatCurrency((summary.totalOnlineCommission || 0) + (summary.totalCashCommission || 0))}
                                            </td>
                                            <td className="py-3 px-4 border">{formatCurrency(summary.totalExpense)}</td>
                                            {/* <td className="py-3 px-4 border text-left text-xs whitespace-pre-wrap">
                                                {formatExpenseReasons(summary)}
                                            </td> */}
                                            <td className="py-3 px-4 border">{summary.customerCount || 0}</td>
                                            <td className="py-3 px-4 border">
                                                <button
                                                    onClick={() => downloadCentreReport(summary.centreId._id)}
                                ...
                                <tr className="border text-center border-gray-600 bg-gray-800 font-bold">
                                    <td className="py-3 px-4 border" colSpan="3">Total</td>
                                    <td className={`py-3 px-4 border ${totals.totalRevenue >= 0 ? "text-green-400" : "text-red-400"}`}>
                                        {formatCurrency(totals.totalRevenue)}
                                    </td>
                                    <td className="py-3 px-4 border">{formatCurrency(totals.totalCash)}</td>
                                    <td className="py-3 px-4 border">{formatCurrency(totals.totalOnline)}</td>
                                    <td className="py-3 px-4 border">{formatCurrency(totals.totalOnlineCommission)}</td>
                                    <td className="py-3 px-4 border">{formatCurrency(totals.grandtotalonline)}</td>
                                    <td className="py-3 px-4 border">{formatCurrency(totals.totalCashCommission)}</td>
                                    <td className="py-3 px-4 border">{formatCurrency(totals.totalPayCom)}</td>
                                    <td className="py-3 px-4 border">{formatCurrency(totals.totalExpense)}</td>
                                    <td className="py-3 px-4 border"></td>
                                    <td className="py-3 px-4 border">{totals.totalCustomerCount}</td>
                                    <td className="py-3 px-4 border"></td>
                                </tr>
                                ...
                                <tr className="border text-center border-gray-600 bg-[#252530] font-bold">
                                    ...
                                    <td className={`py-3 px-4 border ${netProfit >= 0 ? "text-green-400" : "text-red-400"}`}>
